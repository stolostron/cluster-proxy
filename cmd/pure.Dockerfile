...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o kubectlui cmd/kubectl-ui/main.go
...
WORKDIR /
COPY --from=builder /workspace/agent /workspace/manager /workspace/kubectlui ./
USER ${USER_UID}

...
func checkAddonStatus() {
	...
	Eventually(func() error {
		...
		fmt.Fprintf(GinkgoWriter, "[DEBUG] Checking service: cluster-proxy-user in namespace: %s\n", hubInstallNamespace)
		_, err = hubKubeClient.CoreV1().Services(hubInstallNamespace).Get(context.Background(), "cluster-proxy-user", metav1.GetOptions{})
		if err != nil {
			fmt.Fprintf(GinkgoWriter, "[ERROR] Failed to get service cluster-proxy-user: %v\n", err)
			return err
		}
		fmt.Fprintf(GinkgoWriter, "[SUCCESS] Service cluster-proxy-user exists\n")
...
func prepareClusterProxyClient() {
	...
	userServerServiceAddress = "cluster-proxy-user." + hubInstallNamespace + ".svc:9092"
